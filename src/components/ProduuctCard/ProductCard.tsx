...
import { getSafeImageUrl, normalizeProduct } from '../../utils/imageUtils';
...
export const ProductCard: React.FC<Props> = ({ product }) => {
  ...
  const isFavorite = favorites.some(
    f => String(f.itemId) === String(product.itemId),
  );

  const isInCart = cart.some(c => String(c.itemId) === String(product.itemId));
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    if (isInCart) {
      removeFromCart(String(product.itemId));
    } else {
  ...
  const handleAddToFavorites = (e: React.MouseEvent) => {
    ...
    if (isFavorite) {
      removeFromFavorites(String(product.itemId));
    } else {
